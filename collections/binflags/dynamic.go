...
//
//	flags := Dynamic[uint]{}
type Dynamic[T constraints.Integer] map[uint]T
...
func (d Dynamic[T]) IsSet(bitNumber uint) bool {
	size := uint(unsafe.Sizeof(d[0])) * bin.BitsInByte
	...
	return bin.IsSetBit(val, bitIdx)
}
...
func (d Dynamic[T]) Set(bitNumber uint) bool {
	...
	d[byteIdx] = bin.SetBit(d[byteIdx], bitIdx)
