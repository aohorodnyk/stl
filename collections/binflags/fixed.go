...
type Fixed[T constraints.Integer] []T
...
func (s Fixed[T]) IsSet(bitNumber uint) bool {
	...
	size := uint(unsafe.Sizeof(s[0])) * bin.BitsInByte
	...
	return bin.IsSetBit(s[byteIdx], bitIdx)
}
...
func (s Fixed[T]) Set(bitNumber uint) bool {
	...
	bitIdx := uint8(bitNumber % size)
	s[byteIdx] = bin.SetBit(s[byteIdx], bitIdx)
...
func (s Fixed[T]) Unset(bitNumber uint) bool {
	...
	bitIdx := uint8(bitNumber % size)
	s[byteIdx] = bin.UnsetBit(s[byteIdx], bitIdx)
